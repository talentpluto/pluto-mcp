...
`4.14.0` and server contract `4.15.0` (4.12.0 adds search-time
auto-verify via verifyBudget; 4.13.0 adds investor-backed and
deal-recency cohort filters; 4.14.0 removes emails from deep enrichment
and adds the separate five-credit `full_enrich_candidate` operation;
4.15.0 facets full-enrichment web presence across general, code-hosting,
personal-site, and publication searches and adds an optional category
hint to web-presence findings — none of which changes this route's own
tools). On any conflict, prefer the live tool descriptions and schema
field descriptions.
